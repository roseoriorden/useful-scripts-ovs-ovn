...
sudo ovs-vsctl --db=unix:/home/rose/tmp-ovs/conf.sock add-br br-ext
sudo ovs-vsctl --db=unix:/home/rose/tmp-ovs/conf.sock add-port br-ext ovs-main1
# Start new local ovsdb-server with new conf.db.
sudo ovsdb-tool create /home/rose/tmp-ovs/conf2.db /home/rose/ovs/vswitchd/vswitch.ovsschema
sudo ovsdb-server --detach --no-chdir --pidfile=/home/rose/tmp-ovs/conf2.pid -vconsole:off --log-file=/home/rose/tmp-ovs/db-conf2.log -vsyslog:off --remote=punix:/home/rose/tmp-ovs/conf2.sock /home/rose/tmp-ovs/conf2.db --unixctl=/home/rose/tmp-ovs/ovs-vswitchd.ctl
# Start new ovs-vswitchd in ovs-second.
sudo -E ip netns exec ovs-second ovs-vswitchd --detach --no-chdir --pidfile=/home/rose/tmp-ovs/ovs-vswitchd2.pid -vconsole:off --log-file=/home/rose/tmp-ovs/ovs-vswitchd2.log -vsyslog:off --unixctl=/home/rose/tmp-ovs/ovs-vswitchd.ctl unix:/home/rose/tmp-ovs/conf2.sock
# Create a br-ext bridge in a new OVS and add the other veth port there.
sudo ovs-vsctl --db=unix:/home/rose/tmp-ovs/conf2.sock add-br br-ext
sudo ovs-vsctl --db=unix:/home/rose/tmp-ovs/conf2.sock add-port br-ext ovs-second0
# Start a new ovn-controller connecting to a new local ovsdb-server and the OLD sb-db.
sudo ovs-vsctl --db=unix:/home/rose/tmp-ovs/conf2.sock set open_vswitch . external-ids:ovn-remote="unix:/home/rose/tmp-ovn/sb-db.sock"
...
sudo -E ovs-vsctl --db=unix:$OVS_RUNDIR/conf2.sock set open_vswitch . external-ids:system-id="second"
...
sudo ip -netns ovs-second addr add dev br-ext 172.16.0.1
sudo ip -netns ovs-main addr add dev br-ext 172.16.0.2
# Set ovn-encap-ip in local OVS databases equal to these ip addresses accordingly.
sudo -E ovs-vsctl --db=unix:$OVS_RUNDIR/conf.sock set open_vswitch . external-ids:ovn-encap-ip="172.16.0.2" external-ids:ovn-encap-type=geneve
sudo -E ovs-vsctl --db=unix:$OVS_RUNDIR/conf2.sock set open_vswitch . external-ids:ovn-encap-ip="172.16.0.1" external-ids:ovn-encap-type=geneve
# Set external-ids:ovn-bridge-mapping=physnet:br-ex.
sudo -E ovs-vsctl --db=unix:$OVS_RUNDIR/conf2.sock set open_vswitch . external-ids:ovn-bridge-mapping=physnet:br-ex
